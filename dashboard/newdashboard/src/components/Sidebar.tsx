...
export function Sidebar({ activeTab, onTabChange }: SidebarProps) {
  ...
  return (
    <>
      {/* Desktop Sidebar */}
      <aside className="hidden md:flex w-[260px] flex-shrink-0 flex-col p-5 bg-obsidian-800/80 backdrop-blur-3xl border-r border-white/5 z-40">
        {/* Logo */}
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 h-16 bg-obsidian-800/95 backdrop-blur-3xl border-t border-white/10 z-40 flex items-center justify-around px-2 pb-safe">
        {navItems.map((item) => {
          const isActive = activeTab === item.id;
          const Icon = item.icon;
          return (
